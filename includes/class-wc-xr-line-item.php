...
class WC_XR_Line_Item {
	...
	public function get_tax_type() {
		...
		if ( empty( $tax_rate['label'] ) ) {
			...
			/**
			 * Filter the default tax label.
			 *
			 * @since 1.8.5
			 *
			 * @param string $tax_rate['label'] The default tax label.
			 * @param string $base_country The base country.
			 *
			 * @return string The default tax label.
			 */
			$tax_rate['label'] = apply_filters( 'woocommerce_xero_default_tax_label', $tax_rate['label'], $base_country );
	 ...
	 * @version 1.7.7
	 *
	 * @return string   NONE | EXEMPTOUTPUT | EXEMPTEXPENSES | (filtered tax exempt type)
	 */
	protected function get_tax_exempt_type_for_base_country() {
		...
		/**
		 * Filter the tax exempt type for the line item for the country.
		 *
		 * @since 1.8.5
		 *
		 * @param string $tax_exempt_type The tax exempt type.
		 * @param string $base_country The base country.
		 *
		 * @return string The tax exempt type for the line item for the country.
		 */
		return apply_filters( 'woocommerce_xero_tax_exempt_type', $tax_exempt_type, $base_country );
	}
	 ...
	 *
	 * Only AU, NZ and GB have report tax types, or any country added via the filter
	 *
	...
	protected function get_report_tax_type_for_base_country( $create_tax_rate = false ) {
		...
		/**
		 * Filter the report tax type for the line item for the country.
		 *
		 * @since 1.8.5
		 *
		 * @param string $report_tax_type The report tax type.
		 * @param string $base_country The base country.
		 *
		 * @return string The report tax type for the line item for the country.
		 */
		return apply_filters( 'woocommerce_xero_report_tax_type', $report_tax_type, $base_country );
	}
